<template>
	<div class="main">
		...
		<el-table ref="multipleTable" :data="commentsList.filter(data => !search || data.movieName.toLowerCase().includes(search.toLowerCase()))" style="width: 100%"  border   @selection-change="handleSelectionChange">
				...
				<el-table-column prop="userImg" label="评论人头像" width="100" align="center">
				 ...
				 <el-table-column prop="movieName" label="电影" align="center"  width="120"></el-table-column>
				...
				<el-table-column label="审核"  prog="check" :filters="[{text:'已批准',value:true},{text:'审核中',value:false}]" :filter-method="filterCheck" filter-placement="bottom-end" align="center" width="100">
				    	<template slot-scope="scope">
				    		...
				    		<el-button size="mini" type="danger" v-else @click="onCheck(scope.row)">
				    			{{scope.row.check|formatCheck}}
				    		</el-button>
		...
		methods:{
			...
			getData(){
				...
				this.axios.get('api/admin/commentsList').then(response=>{
					let list=response.data.data.filter(o=>{return o.movie_id});
					list.map(o=>{
						this.axios.all([this.axios.post('api/users/userDetail',{username:o.username}),this.axios.post('api/movieDetail',{movie_id:o.movie_id})]).then(this.axios.spread((res1,res2)=>{
								o.user_id=res1.data.data[0]._id;
								o.userImg=res1.data.data[0].userImg;

								o.movieName=res2.data.data.movieName;
